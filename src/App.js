...
function useSpeaker() {
  ...
  const speakNext = () => {
    if (stopped.current || paused.current || queue.current.length === 0) {
      if (queue.current.length === 0 && !stopped.current) {
  ...
  const play = (text) => {
    ...
    queue.current = sentences.flatMap(s =>
      s.length <= 150 ? [s.trim()] : s.split(/,\s+/).map(p => p.trim()).filter(Boolean)
    );
    ...
    <div style={{
      ...
      {!loaded && !error && (
        <div style={{
          ...
          color: "rgba(253,230,138,0.4)", fontSize: 13, fontStyle: "italic",
          minHeight: 180,
        }}>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    style.textContent = `
      ...
      ::-webkit-scrollbar-thumb { background: rgba(253,186,116,0.4); border-radius: 3px; }
      .bar1 { animation: sound-wave 0.8s 0.0s ease-in-out infinite; }
      .bar2 { animation: sound-wave 0.8s 0.15s ease-in-out infinite; }
      .bar3 { animation: sound-wave 0.8s 0.3s ease-in-out infinite; }
      .bar4 { animation: sound-wave 0.8s 0.45s ease-in-out infinite; }
      .bar5 { animation: sound-wave 0.8s 0.6s ease-in-out infinite; }
    `;
...
Make the story vivid, fun, and full of wonder!`;
  ...
  const inputStyle = {
    ...
    border: "1.5px solid rgba(253,186,116,0.3)", borderRadius: 14,
    padding: "14px 18px", color: "#fde68a",
  ...
  const btnPrimary = {
    ...
    borderRadius: 50, padding: "16px 44px", color: "#1a0a3d",
    fontFamily: "'Quicksand', sans-serif", fontSize: 18, fontWeight: 700,
    cursor: "pointer", animation: "pulse-glow 2.5s ease-in-out infinite",
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: "linear-gradient(170deg, #0a0520 0%, #12082e 40%, #1a0a3d 70%, #0d1535 100%)",
      fontFamily: "'Quicksand', sans-serif", position: "relative", overflow: "hidden",
    }}>
      <Stars />
      <div style={{
        position: "fixed", top: 32, right: 60, width: 80, height: 80, borderRadius: "50%",
        background: "radial-gradient(circle at 35% 35%, #fff8c2, #fde68a 60%, #f59e0b)",
        boxShadow: "0 0 40px 12px rgba(253,230,138,0.35), 0 0 80px 30px rgba(253,230,138,0.12)",
        zIndex: 0, pointerEvents: "none",
      }} />

      <div style={{
        position: "relative", zIndex: 1, maxWidth: 560, margin: "0 auto",
        padding: "40px 24px 60px", minHeight: "100vh",
        ...
        <div style={{ textAlign: "center", marginBottom: 8, animation: "fadeSlideUp 0.7s ease both" }}>
          ...
          <h1 style={{
            fontFamily: "'Lora', serif", fontSize: "clamp(26px, 6vw, 36px)", fontWeight: 600,
          ...
          <p style={{ color: "rgba(253,230,138,0.6)", fontSize: 14, margin: 0, fontStyle: "italic" }}>
            A magical bedtime story, just for you
          </p>
        </div>
        ...
        {page === PAGE.FORM && (
          <div style={{ width: "100%", animation: "fadeSlideUp 0.6s 0.1s ease both", opacity: 0 }}>
            ...
            <input style={inputStyle} placeholder="Enter the child's name..."
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, width: "100%" }}>
              {themes.map((t) => (
                <button key={t.id} onClick={() => setTheme(t.id)} style={{
                  background: theme === t.id ? "rgba(253,186,116,0.2)" : "rgba(255,255,255,0.04)",
                  border: `1.5px solid ${theme === t.id ? "rgba(253,186,116,0.7)" : "rgba(255,255,255,0.1)"}`,
                  borderRadius: 12, padding: "12px 10px",
                  color: theme === t.id ? "#fde68a" : "rgba(253,230,138,0.5)",
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, width: "100%" }}>
              {moods.map((m) => (
                <button key={m.id} onClick={() => setMood(m.id)} style={{
                  background: mood === m.id ? "rgba(253,186,116,0.2)" : "rgba(255,255,255,0.04)",
                  border: `1.5px solid ${mood === m.id ? "rgba(253,186,116,0.7)" : "rgba(255,255,255,0.1)"}`,
                  borderRadius: 12, padding: "12px 10px",
                  color: mood === m.id ? "#fde68a" : "rgba(253,230,138,0.5)",
                  fontFamily: "'Quicksand', sans-serif", fontSize: 13, fontWeight: 600,
                  cursor: "pointer", transition: "all 0.2s", textAlign: "left",
                }}>{m.label}</button>
              ))}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, width: "100%" }}>
              {illustrationStyles.map((s) => (
                <button key={s.id} onClick={() => setIllustrationStyle(s.id)} style={{
                  background: illustrationStyle === s.id ? "rgba(253,186,116,0.2)" : "rgba(255,255,255,0.04)",
                  border: `1.5px solid ${illustrationStyle === s.id ? "rgba(253,186,116,0.7)" : "rgba(255,255,255,0.1)"}`,
                  borderRadius: 12, padding: "12px 10px",
                  color: illustrationStyle === s.id ? "#fde68a" : "rgba(253,230,138,0.5)",
                  fontFamily: "'Quicksand', sans-serif", fontSize: 13, fontWeight: 600,
                  cursor: "pointer", transition: "all 0.2s", textAlign: "left",
                }}>{s.label}</button>
              ))}
            ...
            <input style={inputStyle} placeholder="e.g. a tiny dragon, a talking fox..."
              value={sidekick} onChange={(e) => setSidekick(e.target.value)} />

            {error && <p style={{ color: "#fb7185", fontSize: 13, marginTop: 12, textAlign: "center" }}>{error}</p>}

            <div style={{ display: "flex", justifyContent: "center", marginTop: 32 }}>
              <button style={btnPrimary} onClick={generateStory}
        ...
        {page === PAGE.LOADING && (
          <div style={{ textAlign: "center", marginTop: 80 }}>
            ...
            <p style={{ color: "#fde68a", fontSize: 20, fontWeight: 700, marginTop: 20 }}>
              Conjuring your story{".".repeat(dots)}
            </p>
            <p style={{ color: "rgba(253,230,138,0.45)", fontSize: 14, fontStyle: "italic" }}>
              The dream fairies are painting your world
            </p>
          </div>
        )}
        ...
        {page === PAGE.STORY && (
          <div ref={storyRef} style={{ width: "100%" }}>
            ...
            {/* Voice Controls */}
            <div style={{
              background: "rgba(253,186,116,0.08)", border: "1px solid rgba(253,186,116,0.25)",
              borderRadius: 16, padding: "14px 18px", marginTop: 12,
              display: "flex", flexDirection: "column", gap: 10,
            }}>
              <p style={{ color: "#fde68a", fontWeight: 700, fontSize: 12, letterSpacing: 1.5, textTransform: "uppercase", margin: 0, opacity: 0.7 }}>
                🔊 Read Aloud
              </p>
              <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                {!isPlaying ? (
                  <button onClick={() => play(`${storyTitle}. ${story}`)} style={{
                    background: "linear-gradient(135deg, #f59e0b, #fb923c)",
                    border: "none", borderRadius: 50, padding: "11px 26px",
                    color: "#1a0a3d", fontFamily: "'Quicksand', sans-serif",
                    fontSize: 14, fontWeight: 700, cursor: "pointer",
                  }}>▶ Play Story</button>
                ) : (
                  <>
                    <button onClick={togglePause} style={{
                      background: "rgba(253,186,116,0.15)", border: "1.5px solid rgba(253,186,116,0.5)",
                      borderRadius: 50, padding: "11px 22px", color: "#fde68a",
                    ...
                    <button onClick={stop} style={{
                      ...
                      borderRadius: 50, padding: "11px 22px", color: "rgba(253,230,138,0.6)",
                      fontFamily: "'Quicksand', sans-serif", fontSize: 14, fontWeight: 700, cursor: "pointer",
                    }}>■ Stop</button>
                    {!isPaused && (
                      <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
                        {["bar1","bar2","bar3","bar4","bar5"].map((cls) => (
                          <div key={cls} className={cls} style={{
                            width: 3, height: 18, background: "#f59e0b", borderRadius: 2, opacity: 0.8,
                          }} />
                        ))}
                      </div>
                    )}
            ...
            {/* Story Card */}
            <div style={{
              background: "rgba(255,255,255,0.04)", border: "1px solid rgba(253,186,116,0.2)",
              borderRadius: 20, padding: "30px 26px", marginTop: 14,
            }}>
              <div style={{ textAlign: "center", marginBottom: 28 }}>
                <div style={{ fontSize: 32, marginBottom: 8 }}>📖</div>
                <h2 style={{
                  fontFamily: "'Lora', serif", fontSize: "clamp(18px, 5vw, 24px)",
                  fontWeight: 600, color: "#fde68a", margin: 0, fontStyle: "italic", lineHeight: 1.3,
                }}>{storyTitle}</h2>
              ...
              {paragraphs.map((para, i) => (
                <div key={i} style={{ marginBottom: 28 }}>
                  {imagePrompts[i] && (
                    <IllustrationImage prompt={imagePrompts[i]} style={illustrationStyle} />
                  )}
                  <p style={{
                    fontFamily: "'Lora', serif", fontSize: 16, lineHeight: 1.85,
                    color: "rgba(253,230,138,0.85)", margin: 0,
                  }}>{para}</p>
                </div>
              ))}
            ...
            <div style={{ display: "flex", gap: 12, justifyContent: "center", marginTop: 24, flexWrap: "wrap" }}>
              <button onClick={generateStory}
                style={{ ...btnPrimary, padding: "13px 28px", fontSize: 15 }}
                onMouseEnter={(e) => (e.target.style.transform = "scale(1.05)")}
                onMouseLeave={(e) => (e.target.style.transform = "scale(1)")}>
                ✨ New Story
              </button>
              <button onClick={() => setPage(PAGE.FORM)} style={{
                background: "rgba(255,255,255,0.06)", border: "1.5px solid rgba(253,186,116,0.3)",
                borderRadius: 50, padding: "13px 28px", color: "#fde68a",
